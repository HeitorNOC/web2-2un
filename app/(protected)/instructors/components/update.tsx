...
import { updateInstructorSchema } from "@/schemas/index"
...
const UpdateInstructorModal: FC<UpdateInstructorModalProps> = ({ isOpen, instructor, onConfirm, onCancel }) => {
  const [selectedImage, setSelectedImage] = useState<string | null>(null)

  useEffect(() => {
    setSelectedImage(instructor.image || null)
  }, [instructor])

  const { register, handleSubmit, formState: { errors } } = useForm<UpdateInstructorFormData>({
    resolver: zodResolver(updateInstructorSchema),
    defaultValues: {
      name: instructor.name,
      email: instructor.email,
      cpf: instructor.cpf,
      phone: instructor.InstructorAdditionalData?.phone || "",
      cref: instructor.InstructorAdditionalData?.cref || "",
    },
  })
  ...
  const onSubmit: SubmitHandler<UpdateInstructorFormData> = (data) => {
    const formattedData = {
      ...data,
      image: selectedImage,
    }
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onCancel}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>Atualizar Professor</DialogTitle>
          <DialogDescription>Atualize as informações do professor abaixo.</DialogDescription>
        </DialogHeader>
